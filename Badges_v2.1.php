...
/*output image cache*/
$Cache="/tmp/".strtolower(rawurlencode($username))."_$type-$style-$color.png";
...
{
	/**
	 * Do we have any fresh cached badge ?
	 */
	$res = mysql_query("SELECT * FROM badges WHERE username='" . gpc_addslashes(strtolower($username)) . "'");
	$badge = mysql_fetch_assoc($res);
	
	if (!mysql_num_rows($res) OR $badge['lastupdate'] < $data['lastupdate'])
	{
	  //touch_table($username);
		...
		imagepng($img, $Cache);
		$QUERY=sprintf("REPLACE INTO badges (type, style, color, lastupdate, hits, lasthit, png, username) VALUES ('%s','%s','%s',%s, %s, %s, '%s', '%s');", 
			  $type,
			  $style,
			  $color,
			  time(),
			  $hits+1,
			  time(),
			  mysql_real_escape_string(file_get_contents($Cache)),
			  $username);
		//echo $QUERY;
		mysql_query($QUERY);
		unset($Cache);
	...
	$QUERY=sprintf("select png from badges where username = '%s' and type = '%s' and style = '%s' and color = '%s'",
				$username,
				$type,
				$style,
				$color);
